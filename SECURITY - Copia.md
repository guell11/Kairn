...
O navegador Kaggle embutido usa perfil temporário. Cookies, login Google/Kaggle,
OAuth, histórico, armazenamento local e tokens de sessão não são gravados no
projeto. `.browser/`, `.kaggle-studio/`, ambientes, credenciais e chaves também
estão bloqueados pelo `.gitignore`.
...
- **Claude Code:** `apiKeyHelper` lê token de arquivo local protegido; o token não entra em `settings.json`.
- **OpenCode:** a config usa `{file:~/.kaggle-studio-opencode-token}` e o token fica em arquivo local protegido.
